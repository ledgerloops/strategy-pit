...
export class Badger extends EventEmitter implements NetworkNode {
  ...
  protected connectLoopsEngine(traceEngine: TracesEngine): SaigaLoopsEngine {
    ...
    loopsEngine.on('lookup-trace', (spec: { sender: string, probeId: string, traceId: string, legId: string }, callback: (traceTo: string | undefined, equivalent: string[]) => void) => {
    ...
    loopsEngine.on('message', (to: string, message: string) => {
      this.debugLog.push(`[Node#sendTraceMessage] ${this.name} sends loops message to ${to}: ${message}`);
      this.emit('message', to, message);
    });
    return loopsEngine;
  }
