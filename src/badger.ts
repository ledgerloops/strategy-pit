...
export class Badger extends EventEmitter implements NetworkNode {
  ...
  protected connectLoopsEngine(traceEngine: TracesEngine): SaigaLoopsEngine {
    ...
    loopsEngine.on('lookup-trace', (spec: { probeId: string, traceId: string, legId: string }, callback: (traceFrom: string, traceTo: string) => void) => {
      this.debugLog.push(`[Node#lookup-trace] ${this.name} is looking up trace ${spec.probeId} ${spec.traceId} ${spec.legId}`);
      const trace = traceEngine.get(spec);
      if (typeof trace === 'undefined') {
        callback('', '');
      } else {
        callback(trace.from, trace.to);
      }
